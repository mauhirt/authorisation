    ...
    for i in range(3):
        try:
            with urllib.request.urlopen(url,timeout=120) as r:
                b=r.read()
                if not b.strip(): return None      # empty geography (e.g., no secondary SDs)
                return json.loads(b)
...
for vint in ("2010","2019"):
    for st,fips in STATES.items():
        for geo in ("county","place","school district (unified)","school district (elementary)","school district (secondary)"):
            cols=",".join(["NAME"]+list(V)+A65)
            u=(f"https://api.census.gov/data/{vint}/acs/acs5?get={cols}"
               f"&for={urllib.parse.quote(geo)}:*&in=state:{fips}&key={KEY}")
            data=get(u)
            if not data:
                print(f"  skip {vint} {st} {geo}: empty"); continue
            hdr=data[0]
            for row in data[1:]:
                d=dict(zip(hdr,row))
                p65=sum(float(d[a] or 0) for a in A65)
                out.append([vint,fips,geo,d.get(geo) or d.get(geo.replace("school district (","school district (").strip()) or list(d.values())[-1],d["NAME"]]+[d[k] for k in V]+[f"{p65:.0f}"])
