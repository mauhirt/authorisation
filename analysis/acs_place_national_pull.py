#!/usr/bin/env python3
"""National city panel support — ACS5 2019 place-level covariates, ALL states.
(The 9-state acs_covariates.csv pull generalized; one call per state.)
Usage: CENSUS_KEY=... python3 analysis/acs_place_national_pull.py
Writes analysis/cache/acs_place_national.csv:
  state_fips, place_fips5, name, homeown, share65, frac, medinc, pop
...
out=[["state_fips","place_fips5","name","homeown","share65","frac","medinc","pop"]]
cols=",".join(["NAME"]+list(V)+A65)
for st in STATES:
    u=(f"https://api.census.gov/data/2019/acs/acs5?get={cols}&for=place:*&in=state:{st}&key={KEY}")
    js=get(u)
